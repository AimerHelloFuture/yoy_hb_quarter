...
class BaseStockHB(BaseStockSheet):
    """
    财务指标的环比计算（单季环比）
    """
        ...
        if all_com_codes:
            for index, com_code in enumerate(all_com_codes):
                if cls.mode == 'increase':  # 增量更新
                    ...
                    # 如果是新增公司，第一次就跑全量
                    if find_new_com:
                        cls.work_com_hb(com_code, consolidation=1501002)
                    else:
                        cls.work_com_hb_increase(com_code, consolidation=1501002)
    ...
    @classmethod
    def work_com_hb(cls, com_code, consolidation):
        """
        ...
        logging.info('perform com code of hb '+str(com_code))
            ...
            for index, record in enumerate(records):
                ...
                for i in range(0, len(records)):
                    if str(records[i][u'end_date']) == initial_account_date:
                        ...
                        break
                cls.update_indicator_hb(record, last_record)
    ...
    @classmethod
    def work_com_hb_increase(cls, com_code, consolidation):
        """
        增量更新
        ...
        if end_date_increase_scope:
            for index, end_date_increase in enumerate(end_date_increase_scope):
                cls.update_indicator_hb(com_code, end_date_increase, consolidation)
            index += 1
        logging.info('finish com code {0} increase {1} records'.format(com_code, index))

    @classmethod
    def update_indicator_mysql(cls, record, last_record, indicators_dict=dict(), upsert=False):
        """
        ...
        for indicator, value in indicators_dict.items():
            if indicator not in cls.fields_columns_tuple2:
                indicators_dict[indicator] = cls.get_hb_rate_new(record, last_record, indicator)
    ...
    @classmethod
    def update_indicator_hb(cls, record, last_record, upsert=False):
        """
        更新指标的hb值
        :param record:
        :param last_record:
        ...
        cls.update_indicator_mysql(record, last_record, indicators_dict, upsert)
